...
# Impostiamo il layout su "wide" per dare più respiro su PC, su mobile si adatta
...
st.markdown("""
    ...
    /* Questo è il trucco per fare la cornice su mobile */
    ...
    /* I titoli devono essere scuri perché sono su sfondo bianco */
    h1, h2, h3, h4 {
...
with col_logo:
    ...
    st.markdown("<h1 style='text-align: center; color: #D4AF37 !important;'>⚖️</h1>", unsafe_allow_html=True)
with col_title:
...
""")
st.sidebar.markdown("<div style='text-align:center; color: #D4AF37; margin-top: 20px;'>⚜️ Luxury Edition v3.0</div>", unsafe_allow_html=True)
...
with tab1:
    ...
    compenso_lordo = st.number_input("Compensi Lordi Annui (€)", min_value=0.0, value=30000.0, step=1000.0, format="%.2f")
    ...
    if st.button("Calcola Netto", key="btn_netto"):
        ...
        <div class="result-card">
            ...
            <h1 style="margin:0; color:#002a52; font-size: 2.5em;">€ {netto_finale:,.2f}</h1>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.info(f"ℹ️ **Nota Bene:** In fattura dovrai aggiungere il **4% C.P.A.** (pari a € {cassa_integrativa:,.2f}). Questa somma la incassi dal cliente ma la devi versare interamente alla Cassa, quindi non è reddito tuo.")
...
with tab2:
    ...
    netto_desiderato = st.number_input("Netto Desiderato Annuo (€)", min_value=0.0, value=24000.0, step=1000.0, format="%.2f")
    ...
    if st.button("Calcola Fatturato Necessario", key="btn_lordo"):
        ...
        <div class="result-card" style="border-left-color: #D4AF37;">
            ...
            <h1 style="margin:0; color:#D4AF37; font-size: 2.5em;">€ {lordo_necessario:,.2f}</h1>
            <small style="color: #666">+ 4% CPA in fattura = Totale bonifico cliente € {lordo_necessario*1.04:,.2f}</small>
        </div>
        """, unsafe_allow_html=True)
