...
st.set_page_config(page_title="Studio Gaetani | Avvocati Tax Simulator", page_icon="⚖️", layout="centered")
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.markdown("<h1 style='font-size: 60px; text-align:center;'>⚖️</h1>", unsafe_allow_html=True)
...
st.markdown("""
    ...
    /* === FIX CRITICO INPUT VISIBILITY === */
    
    /* 1. Colore del testo che scrivi (Numeri, Lettere) -> BIANCO */
    input, textarea, .stNumberInput input, .stTextInput input, .stTextArea textarea {
        color: #ffffff !important; 
        caret-color: #D4AF37 !important; /* Cursore Oro */
        font-weight: 500;
    }

    /* 2. Sfondo dei box di input -> Blu più chiaro per contrasto */
    div[data-baseweb="input"] > div, 
    div[data-baseweb="base-input"], 
    textarea {
        background-color: #002a52 !important; /* Blu medio */
        border: 2px solid #D4AF37 !important; /* Bordo Oro */
        border-radius: 5px;
    }

    /* 3. Etichette sopra gli input (es. "Compensi Lordi") -> ORO */
    ...
    /* CARD RISULTATI (Bianca con testo scuro) */
    .result-card {
...
# --- TABS ---
tab1, tab2, tab3 = st.tabs(["📉 Dal Lordo al Netto", "🎯 Dal Netto al Lordo", "📝 Genera Fattura"])

# === TAB 1: LORDO -> NETTO ===
with tab1:
    st.markdown("### Calcolo Redditività")
    st.write("Inserisci gli onorari annui incassati.")
    lordo_input = st.number_input("Compensi Lordi Annuo (€)", min_value=0.0, value=30000.0, step=1000.0)
    ...
    if st.button("CALCOLA NETTO", key="btn_avv_normale"):
        ...
        st.warning(f"⚠️ **Nota Integrativo:** In fattura dovrai aggiungere il 4% CPA (€ {cpa:,.2f}). Questa somma la incassi dal cliente ma la versi interamente alla Cassa.")

# === TAB 2: NETTO -> LORDO ===
with tab2:
    st.markdown("### Pianificazione Obiettivi")
    st.write("Quanto vuoi guadagnare 'pulito' in tasca?")
    netto_input = st.number_input("Netto Desiderato Annuo (€)", min_value=0.0, value=24000.0, step=1000.0)
    ...
    if st.button("CALCOLA FATTURATO NECESSARIO", key="btn_avv_inverso"):
        ...
        st.write("#### 🔢 Dimostrazione del calcolo")
...
with tab3:
    st.markdown("### 📝 Generatore Fattura Pro-forma")
    ...
    with st.form("form_fattura_avv"):
        ...
        desc = st.text_input("Oggetto / Descrizione Prestazione", "Attività di consulenza legale stragiudiziale...")
        onorari = st.number_input("Onorari / Compensi (€)", min_value=0.0, value=1000.0)
        
        if st.form_submit_button("📄 GENERA PDF FATTURA"):
            ...
            st.write(f"Onorari: € {onorari:.2f} | CPA 4%: € {cpa_val:.2f} | **Totale Documento: € {tot_lordo:.2f}** | Bollo: € {bollo_val:.2f}")
            st.markdown(f"### Totale da Pagare: € {tot_pagare:.2f}")
            
            try:
                pdf_bytes = create_pdf_avvocato(dati_pdf)
                b64 = base64.b64encode(pdf_bytes).decode()
                href = f'<a href="data:application/octet-stream;base64,{b64}" download="Fattura_Avv_{num.replace("/","_")}.pdf" style="background-color: #D4AF37; color: #001529; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">📥 SCARICA PDF</a>'
