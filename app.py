...
# --- CSS PARTE 2: INPUT & BUTTONS (Diviso per sicurezza) ---
st.markdown("""
    <style>
    /* INPUT BIANCHI SU BLU */
...
with col_title:
    ...
    try:
        ...
        st.markdown(f"<div style='background-color: #002a52; padding: 5px 10px; border-radius: 5px; display: inline-block; border: 1px solid #D4AF37;'><small>Area Riservata Clienti</small></div>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.subheader("📝 Dati Previdenziali")
st.sidebar.info("""
**Attività:** Studio Legale
**Codice ATECO:** 69.10.10
**Coeff. Redditività:** 78%
**Cassa Forense:** 15% (Su Reddito)
""")
...
with tab1:
    ...
    if st.button("CALCOLA NETTO", key="btn_avv_normale"):
        ...
        st.table(genera_tabella_avvocato(risultati, aliquota_tassa).style.format({"Importo (€)": "€ {:,.2f}"}))
        st.warning(f"⚠️ **Nota:** In fattura dovrai aggiungere il 4% CPA. Questa somma la incassi ma la versi alla Cassa.")
...
with tab2:
    ...
    if st.button("CALCOLA FATTURATO", key="btn_avv_inverso"):
        ...
        """, unsafe_allow_html=True)
        st.write("#### 🔢 Verifica")
        st.table(genera_tabella_avvocato(calcoli_avvocato(lordo_nec, aliquota_tassa), aliquota_tassa).style.format({"Importo (€)": "€ {:,.2f}"}))
        if lordo_nec > 85000: st.error("⚠️ ATTENZIONE: Superi il limite di € 85.000!")
...
with tab3:
    ...
    with st.form("form_fattura_avv"):
        ...
        desc = st.text_input("Oggetto", "Attività di consulenza legale...")
        onorari = st.number_input("Onorari (€)", min_value=0.0, value=1000.0)
        
        if st.form_submit_button("📄 GENERA PDF"):
